...
echo "---------------------------------------------------" | tee -a $logfile
echo "- STEP 1 : local mount cadenceed on the system    -" | tee -a $logfile
echo "---------------------------------------------------" | tee -a $logfile   
echo " " | tee -a $logfile
...
clear
echo "  ==============================================  "
echo "  .:: CAD Support Refresh and deploy script  ::.  "
echo "  ==============================================  "
echo "	"

echo "  Please select a task underneath: "
echo "	"
echo "  1 > Deploy Cadence platform release"
echo "  2 > Deploy design environment"
echo "  3 > Deploy Foundry Development Kit release"
echo "  4 > Deploy DSS environment on the system"
echo "  5 > Exit menu" 
echo "  "
echo -n "  Make your choise (1..5): "
set menuchoise = $<
if ($menuchoise == 1) then
	goto basereleasemenu
else
	if ($menuchoise == 2) then
		goto deploydesignenv	
	else
		if ($menuchoise == 3) then
			goto deployfdk
		else	
			if ($menuchoise == 4) then
				goto deploydss
			else
				if ($menuchoise == 5) then
					cd $curpath
					umount -l /mnt/ed-prod
					exit
				else
					echo "Invalid choise! Please select a valid option!"
					goto mainmenu
				endif
			endif
		endif
